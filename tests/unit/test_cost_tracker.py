...
def test_budget_soft_limit_warns_but_allows() -> None:
    run_id = uuid4()
    tracker = CostTracker()
    tracker.add_record(
        CostRecord(
    ...
    budget = BudgetConstraint(max_total=Decimal("0.20"), soft_limit=Decimal("0.10"), currency="USD")
    can_spend, message = BudgetGuard().can_spend(
        spent_total=tracker.spent_total(run_id),
        planned_cost=Decimal("0.05"),
        budget=budget,
    )
    assert can_spend is True
    assert "reaches soft budget" in message


def test_budget_passes_under_soft_limit() -> None:
    run_id = uuid4()
    tracker = CostTracker()
    tracker.add_record(
        CostRecord(
    ...
    budget = BudgetConstraint(max_total=Decimal("1.00"), soft_limit=Decimal("0.50"), currency="USD")
    can_spend, message = BudgetGuard().can_spend(
        spent_total=tracker.spent_total(run_id),
        planned_cost=Decimal("0.05"),
        budget=budget,
